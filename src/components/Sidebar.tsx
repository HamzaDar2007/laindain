...
const Sidebar: React.FC = () => {
    ...
    const getIcon = (iconName: string) => {
        const icons: Record<string, JSX.Element> = {
            ...
            accounts: (
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 01-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />
            ),
            journal: (
    ...
    return (
        <aside
            className={`${sidebarOpen ? 'w-64' : 'w-0'} bg-gray-900 border-r border-gray-800 flex flex-col transition-all duration-300 ease-in-out relative overflow-hidden`}
        >
                ...
                <nav className="space-y-1 px-3">
                    {navItems.map((item) => (
                        <NavLink
                            ...
                            to={item.path}
                            className={({ isActive }) =>
                                `flex items-center space-x-3 px-3 py-3 mx-2 rounded-lg transition-all duration-200 group whitespace-nowrap overflow-hidden ${isActive
